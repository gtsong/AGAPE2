...
int main(int argc, char *argv[])
{
	...
	else if( (argc == 4) && (strcmp(argv[3], "GENE_NAME_FIRST") == 0 ))  {
		is_gname_given = true;
		is_gname_first = true;
	}
	else if( argc != 3 ) {
		...
		if((buf[0] == '>') || (buf[0] == '<'))
		{
			...
			if( is_gname_given == true ) {
				if( sscanf(buf+1, "%s %s %s %*s", scaf_name, loc, name) != 3 ) {
					...
					if( is_gname_first == true ) {
						if( strcmp(scaf_name, argv[2]) == 0 ) is_end = 1;
					}
					else if( strcmp(name, argv[2]) == 0 ) is_end = 1;
				}
			}
			else {
