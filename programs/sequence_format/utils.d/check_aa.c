...
int main(int argc, char **argv) {
	...
	if( argc == 4 ) {
		...
		else if( strcmp(argv[3], "BOOLEAN_OUTPUT") == 0 ) {
			is_boolean_out = true;
		}
		else if( strcmp(argv[3], "INFO_OUTPUT") == 0 ) {
		...
		if( buf[0] == '>' ) {
			if( count != 0 ) {
				if( (is_correct_start == true) && (is_wrong_splicing == false) && (is_correct_stop == true ) ) {
					if ( is_boolean_out == true ) printf("true");
					else if( is_internal_stops == false) 
						print_genes(b, e, genes, num_genes, exons, num_exons, "gene", shift_b, shift_e, is_internal_stops);
				}				
				else {
					if ( is_boolean_out == true ) printf("false");
					else if( is_internal_stops == false) 
