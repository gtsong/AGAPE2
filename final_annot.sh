...
mkdir -p $out_dir/final/temp
temp_dir=$out_dir/final/temp
mkdir -p $out_dir/final/gff
mkdir -p $out_dir/final/cds
mkdir -p $out_dir/final/aa

rm -rf $out_dir/final/gff/$strain_name.gff
rm -rf $out_dir/final/cds/$strain_name.cds.fasta
rm -rf $out_dir/final/aa/$strain_name.aa.fasta

echo "#" > $out_dir/final/temp.codex
less $fasta | grep ">" | sed 's/>//g'  > $out_dir/final/scf.list
...
do
	...
	then
		...
		then
			...
			$BIN/check_aa $temp_dir/temp.genes.gff $temp_dir/genes.aa LAST_COLUMN | grep "gene\|CDS" > $temp_dir/$scf_name.genes.gff
		fi
		num=`less $temp_dir/$scf_name.genes.gff | grep -w "gene" | wc -l`
	else
		num=0
	fi
	...
	if [ $num -gt 0 ]
	then
		num_genes=`less $temp_dir/$scf_name.genes.gff | grep -w "gene" | wc -l`
		#less $temp_dir/$scf_name.valid.genes.gff >> $out_dir/final_gff/$strain_name.non_ref.gff
		$BIN/gff2codex $temp_dir/$scf_name.genes.gff CDS_NUM $count > $temp_dir/temp.codex
		less $temp_dir/$scf_name.genes.gff >> $out_dir/gff/$strain_name.gff
